...
if [[ $1 == 'startup' ]]; then
    ...
    if [[ -s /home/savvy/customer_info ]]; then
        SSID2="$(grep ssid /home/savvy/customer_info | awk '{print $2}')" #customer wifi ssid
        PASS2="$(grep wifiPassword /home/savvy/customer_info | awk '{print $2}')" #customer wifi password
        if [[ $SSID2 = 'fake' ]]; then
            SSID2=''
            PASS2=''
        fi
    ...
    wificleanup () {
        ...
        CURWIFI=$(nmcli con show | grep wifi | awk '{print $1}')
        until [[ $CURWIFI = '' ]]
        do
...
if [[ $1 == 'network_status' ]]; then
    ...
    if [ "$NETWORK" = '' ]; then
        ...
        nmcli con show | grep wifi | awk '{print $1}' | while read wifi; do
            IFNAMEPROFILE="$(nmcli con show $wifi | grep interface-name | awk '{print $2}')"
            if [[ $DONGLE != $IFNAMEPROFILE ]]; then
                #get password from profile
                PASSWORD=`nmcli con show $wifi --show-secrets | grep wireless-security.psk: | awk '{print $2}'`
                echo "dongle wrongle $wifi"
                #remove existing wifi profile
                nmcli con delete $wifi

                #add new wifi profile with new dongle
                setupwifi $wifi $PASSWORD
                sleep 1
                #connect to profile if SSID is broadcasting
                if [[ "$(nmcli device wifi list | grep $wifi)" ]]; then
                    nmcli con up $wifi
                    sleep 1
                fi
            fi
...
if [[ $1 == 'update' ]]; then
    ...
    if [[ $2 == 'git' ]]; then
        ...
        git pull > /home/savvy/ea02c68f0b34aa4/git_pull_output
    ...
    if [[ $2 == 'customer' ]]; then
        if [[ -s /home/savvy/.url ]]; then
            ...
            if [[ $? == 0 ]]; then
                ...
                #remove commas, colons, quotes and format json in columns
    ...
    if [[ $2 == 'provision' ]]; then
        . /home/savvy/savvy.sh startup
        CURWIFI=$(nmcli con show | sed '2q;d' | awk '{print $1}')
